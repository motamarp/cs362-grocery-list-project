...
function MealPlan() {
  ...
  const handleGenerateSmartPlan = async () => {
    if (!weekStartDate) {
    setFlashMessage("error", "Please select a start date.");
    return;
    } 
    if (!user) {
    setFlashMessage("error", "You must be logged in.");
    return;
    }
    ...
    try {
      ...
      setWeekStartDate("");
      await fetchPlans();
      setFlashMessage("success", "Smart meal plan created!");
    } catch (err) {
      console.error("Generate failed:", err.response?.data || err);
      setFlashMessage("error", "Could not generate meal plan.");
    } finally {
      setGenerating(false);
    }
  };
    ...
    <div>
      ...
      <div className="panel form-panel">
        ...
        <form className="grid-form" onSubmit={handleCreate}>
          ...
          <div className="form-actions full-span" style={{ display: 'flex', gap: '1rem' }}>
            <button type="submit" className="primary-btn" disabled={loading}>
              {loading ? "Creating..." : "Create Empty Plan"}
            </button>
            <button
              type="button"
