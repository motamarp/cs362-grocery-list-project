...
function MealPlanDetail() {
  ...
  const setFlashMessage = (type, text) => {
    ...
    window.clearTimeout(window.__lettuceDetailFlashTimer);
    window.__lettuceDetailFlashTimer = window.setTimeout(() => {
      setMessage(null);
    }, 2500);
  };
  ...
  const handleGenerateGroceryList = async () => {
    ...
    try {
      await generateGroceryList(id);
      await fetchMealPlan();
      setFlashMessage("success", "Grocery list generated!");
    } catch (err) {
      console.error("Generate grocery list failed:", err.response?.data || err);
      setFlashMessage("error", err.response?.data?.error || "Could not generate grocery list.");
    } finally {
